...
SERIAL CONNECTION PANEL: You can connect via serial to a pic32 agent.  In
theory, the correct port will already be detected, but in practice that's rarely
the case.  Use the text box to manually enter in the port of your Pic(you can
use CoolTerm or something to scan for the correct port, it's usually something
like "COM4").  
...
self-explanitory, they maintain a record of the serial i/o with the pic.  Note
...
#			self.ser_port_box.delete(0,tk.END)
	...
	def sendRES(self):
		...
		if(self.autoplay.get()=="SECOND_MOVE"):
			...
			self.ships_to_sink = self.ships_to_sink[1:] + 
                [self.ships_to_sink[0]]
		if(self.autoplay.get()=="FIRST_MOVE"):
			res_mess = "RES,%s,%s,%d" %(
