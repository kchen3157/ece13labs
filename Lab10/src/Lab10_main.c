...
void Transmission_ReceiveChar(void)
{
    ...
    HAL_StatusTypeDef recvStatus = HAL_UART_Receive(
        &huart2,
        (uint8_t*)&incoming_char, 1, 0
    );
...
int main()
{
    ...
    //Print a greeting:
    OLED_DrawString("This is BattleBoats!\nPress BTN4 to\nchallenge, or wait\nfor opponent.");
    OLED_Update();
    ...
    while (TRUE) {
        ...
        LEDs_Set(1 << AgentGetState());
        HAL_Delay(1);
    }
}

/**
 * This is the interrupt for the Timer3 peripheral. It just keeps incrementing a counter used to
