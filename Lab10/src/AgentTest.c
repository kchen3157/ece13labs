...
/* Negotiation submodule stubs */
typedef uint16_t NegotiationData;
typedef enum { heads = 0, tails = 1 } NegotiationOutcome;

/* Compute beef_hash(x) = (x * x) % 0xBEEF */
NegotiationData NegotiationHash(NegotiationData x) {
    uint32_t t = (uint32_t)x * (uint32_t)x;
    return (NegotiationData)(t % 0xBEEF);
}

/* Always return heads so challenger always goes first */
NegotiationOutcome NegotiateCoinFlip(NegotiationData a, NegotiationData b) {
    (void)a; (void)b;
    return heads;
}

/* In accepting state, agent checks if (!NegotiationVerify) to detect cheating.
   Return false here to force the cheating branch. */
bool NegotiationVerify(NegotiationData a, NegotiationData commit) {
    (void)a; (void)commit;
    return false;
}

/* OLED stubs (drawDialog uses these) */
typedef enum { oled_color_black = 0 } OledColor;
void OLED_Clear(OledColor c)   { (void)c; }
void OLED_DrawString(const char *s) { (void)s; }
void OLED_Update(void)         { /* nothing */ }

/* Field and fieldOled stubs */
typedef struct Field Field;
typedef enum {
    field_oled_turn_none = 0,
...
/* Initialize two field objects (no-op) */
void FieldInit(Field *f1, Field *f2) {
    (void)f1; (void)f2;
}

/* Randomly place all boats (no-op) */
void FieldAIPlaceAllBoats(Field *f) {
    (void)f;
}

/* Always return the same guess so tests are deterministic */
GuessData FieldAIDecideGuess(const Field *f) {
    (void)f;
    return (GuessData){ .row = 1, .col = 2, .result = 0 };
}

/* Update opponent’s field knowledge (no-op) */
void FieldUpdateKnowledge(Field *f, const GuessData *g) {
    (void)f; (void)g;
}

/* Register an enemy attack on my_field (no-op) */
void FieldRegisterEnemyAttack(Field *f, const GuessData *g) {
    (void)f; (void)g;
}

/* Return non-zero so game does not end early */
uint8_t FieldGetBoatStates(const Field *f) {
    (void)f;
    return 1;
}

/* Draw both fields on oled (no-op) */
void FieldOledDrawScreen(const Field *m, const Field *o,
                         FieldOledTurn t, uint8_t c) {
