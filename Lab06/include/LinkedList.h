 ...
 * @file    LinkedList.h
 * 
 * This file provides a doubly-linked list implementation for storing strings
 * (NULL-terminated character arrays).
 *
 * The list implementation relies on a chain metaphor: a list is merely a
 * sequence of links (ListItems) and there is no separate construct to represent
 * the entire list, each ListItem in it does that implicitly.
 * ListItems can store pointers to strings, but the strings themselves must be
 * stored somewhere else. This list supports NULL pointers as well.
 *
 * @author  HARE Lab
 * @author  jLab
 * @version 1.0.0
